...
export default async function addFramework(
  directory: string,
  framework: string,
) {
  ...
  if (framework == "natico") {
    ...
    console.log(
      `${red("!")} We can't add ${
        blue(framework)
      } files, because we don't have a template for that!\n${
        red("!")
      } Weird, since this shouldn't happen. Make a bug report at: https://github.com/Reboot-Codes/create-discordeno-bot/issues`,
    );
    Deno.exit(1);
  }

...
export default async function addBase(directory: string, botType: string, token?: string) {
  ...
  if (botType == 'gateway') {
    ...
    // Create `deps.ts`
    await Deno.writeFile(`${directory}/deps.ts`, encoder.encode(`export * from 'https://deno.land/x/discordeno@13.0.0-rc35/mod.ts';\nexport { config as dotEnvConfig } from "https://deno.land/x/dotenv@v3.1.0/mod.ts";\nexport * from "https://deno.land/std@0.117.0/fmt/colors.ts";\n`)).catch((err) => {
      spinner.fail('Failed to add base files!');
      console.log(`${red('!')} Write file error: Could not write ${blue('deps.ts')} to ${blue(directory)}! Error:\n${err}`);
      Deno.exit(1);
    });

    // Create `config.ts`
    await Deno.writeFile(`${directory}/config.ts`, encoder.encode(`import { dotEnvConfig } from './deps.ts';\ndotEnvConfig({ export: true });\nexport const BOT_TOKEN = Deno.env.get("BOT_TOKEN") || "";\nexport const BOT_ID = BigInt(atob(BOT_TOKEN.split(".")[0]));\n`)).catch((err) => {
      spinner.fail('Failed to add base files!');
      console.log(`${red('!')} Write file error: Could not write ${blue('config.ts')} to ${blue(directory)}! Error:\n${err}`);
      Deno.exit(1);
    });

    // Create `mod.ts`
    await Deno.writeFile(`${directory}/mod.ts`, encoder.encode(`import { ActivityTypes, createBot, startBot } from './deps.ts';\nimport { BOT_ID, BOT_TOKEN } from "./config.ts";\nimport { logger } from "./src/utils/logger.ts";\n\nconst log = logger({ name: "Main" });\n\nlog.info("Starting Bot, this might take a while...");\n\nexport const bot = createBot({\n  token: BOT_TOKEN,\n  botId: BOT_ID,\n  intents: [],\n  events: {}\n});\n\nbot.gateway.presence = {\n  status: "online",\n  activities: [\n    {\n      name: "Discordeno is Best Lib",\n      type: ActivityTypes.Game,\n      createdAt: Date.now(),\n    }\n  ]\n};\n\nawait startBot(bot);\n`)).catch((err) => {
      spinner.fail('Failed to add base files!');
      console.log(`${red('!')} Write file error: Could not write ${blue('mod.ts')} to ${blue(directory)}! Error:\n${err}`);
      Deno.exit(1);
    });
    ...
    await ensureDir(`${directory}/src`).catch((err) => {
      spinner.fail('Failed to add base files!');
      console.log(`${red('!')} We can't create ${blue(`${directory}/src`)}! Error:\n${err}`);
      Deno.exit(1);
    });
  ...
  await Deno.writeFile(`./${directory}/.env`, encoder.encode(`BOT_TOKEN="${token}"\n`))
    .catch((err) => {
      spinner.fail('Failed to add base files!');
      console.log(`${red('!')} Write file error: Could not write ${blue('.env')} to ${directory}! Error:\n${err}`);
      Deno.exit(1);
    });
  // Add .gitignore
  await Deno.writeFile(`./${directory}/.gitignore`, encoder.encode(`.env\n`))
    .catch((err) => {
      spinner.fail('Failed to add base files!');
      console.log(`${red('!')} Write file error: Could not write ${blue('.gitignore')} to ${directory}! Error:\n${err}`);
      Deno.exit(1);
    });

...
export default async function initGit(directory: string) {
  ...
  if (!(gitVersion.output.includes("git version"))) {
    ...
    console.log(
      `${yellow("!")} Could not find the git executable!\n${
        yellow("!")
      } This is not fatal, as this doesn't affect bot operation.`,
    );
    return;
  }
  ...
  if (!directoryExists) {
    ...
    console.log(
      `${
        red("!")
      } We can't create a git repo in ${directory} because it doesn't exist!\n${
        red("!")
  ...
  await exec(`git init ./${directory}`, { output: OutputMode.Capture })
    .catch((err) => {
      spinner.fail("Failed to initialize the Git repository!");
      console.log(
        `${yellow("!")} We can't create a git repo in ${directory}!\n${
          yellow("!")
        } This is not fatal, as this doesn't affect bot operation. Error:\n${err}`,
      );
      return;
    });
